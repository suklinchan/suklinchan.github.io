...
var crash_categories = ["tota", "speeding", "alcohol", "distracted", "previous"];

var dropdown_options = ["Total crashes",
                        "Speeding Crashes", 
                        "Alcohol-impaired Driving Crashes", 
                        "Crashes due to Distractions",  
                        "Drivers had been involved in previous accidents"];

var axis_labels = ["Total Crashes per billion miles", 
...
var tooltip_labels = ["Total Crashes",
                      "Speeding Crashes", 
                      "Alcohol-related", 
                      "Distracted Driving", 
                      "Previous Accidents"];
...
var makeVis = function(data, category) {
  ...
  var yScale = d3.scale.linear()
        .domain([ d3.min(data, function(d) { return d.ins_premium; }) - 1,
                  d3.max(data, function(d) { return d.ins_premium; }) + 1 ])
  ...
  console.log(axis_labels[category])
  ...
  var tipMouseover = function(d) {
      ...
      var html  = "<span style='color:" + color + ";'>" + d.state + "</span><br/>" +
                  "<b>" + xVal + "</b> " + tooltip_labels[category] + ", <br/>" + 
                  "Insurance Premium " + "<b>$" + d.ins_premium + "</b>" + ", <br/>" + 
                  "Insurance Payout " + "<b>$" + d.ins_losses + "</b>";
  ...
  switch (category) {
     case 0:
       ...
       .enter().append("circle")
         ...
         .attr("cy", function(d) { return yScale( d.ins_premium); })  // y position
     ...
     case 1:
       ...
       .enter().append("circle")
         .attr("class", "dot")
         .attr("r", 5.5) // radius size, could map to another data dimension
         .attr("cx", function(d) { return xScale( d.speeding ); })     // x position
         .attr("cy", function(d) { return yScale( d.ins_premium); })  // y position
         // .style("fill", function(d) { return colorScale(d.state); })
         .style("fill", color)
         .on("mouseover", tipMouseover)
     ...
     case 2:
      ...
      .enter().append("circle")
        .attr("class", "dot")
        .attr("r", 5.5) // radius size, could map to another data dimension
        .attr("cx", function(d) { return xScale( d.alcohol ); })     // x position
        .attr("cy", function(d) { return yScale( d.ins_premium); })  // y position
        // .style("fill", function(d) { return colorScale(d.state); })
        .style("fill", color)
        .on("mouseover", tipMouseover)
    ...
    case 3:
      ...
      .enter().append("circle")
        .attr("class", "dot")
        .attr("r", 5.5) // radius size, could map to another data dimension
        .attr("cx", function(d) { return xScale( d.distracted ); })     // x position
        .attr("cy", function(d) { return yScale( d.ins_premium); })  // y position
        // .style("fill", function(d) { return colorScale(d.state); })
        .style("fill", color)
        .on("mouseover", tipMouseover)
    ...
    case 4:
      ...
      .enter().append("circle")
        .attr("class", "dot")
        .attr("r", 5.5) // radius size, could map to another data dimension
        .attr("cx", function(d) { return xScale( d.previous ); })     // x position
        .attr("cy", function(d) { return yScale( d.ins_premium); })  // y position
        // .style("fill", function(d) { return colorScale(d.state); })
        .style("fill", color)
        .on("mouseover", tipMouseover)
